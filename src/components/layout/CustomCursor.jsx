    ...
    const handleHoverStart = (e) => {
      for (const [selector, lbl] of Object.entries({ a: 'View', button: 'Click', input: 'Type', textarea: 'Type' })) {
        if (e.target.closest(selector)) {
          setLabel(lbl)
          if (outerRef.current) outerRef.current.style.borderColor = 'rgba(78,38,226,0.35)'
          return
        }
      }
    }
  ...
  return (
    <div style={{ cursor: 'none' }}>
      ...
      <div
        ...
        style={{
          width: 4, height: 4,
          background: 'rgba(78,38,226,1)',
          borderRadius: '50%',
          boxShadow: '0 0 10px rgba(78,38,226,0.8), 0 0 20px rgba(78,38,226,0.4)',
          willChange: 'transform',
      ...
      {label && (
        ...
        >
          <span className="text-[8px] font-mono text-blood/60 tracking-[0.2em] uppercase bg-black/80 px-2 py-0.5"
            style={{ border: '1px solid rgba(78,38,226,0.1)' }}>
            {label}
          </span>
        </div>
      )}
