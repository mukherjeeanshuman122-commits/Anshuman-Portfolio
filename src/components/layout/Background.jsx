      ...
      {!isMobile && (
        <>
          ...
          <div className="absolute" style={{
            ...
            filter: 'blur(90px)',
            animation: getReducedMotion() ? 'none' : 'blobFloat1 40s ease-in-out infinite reverse',
          }} />
          <div className="absolute" style={{
            top: '10%', right: '20%', width: '25vw', height: '25vw',
            background: 'radial-gradient(circle, rgba(168,85,247,0.07) 0%, transparent 55%)',
            filter: 'blur(80px)',
            animation: getReducedMotion() ? 'none' : 'blobFloat2 32s ease-in-out infinite reverse',
          }} />
        </>
      )}
    </div>
    ...
    <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
      {particles.map((p) => (
        <div key={p.id} className="absolute rounded-full" style={{
          ...
          left: `${p.x}%`, top: `${p.y}%`,
          opacity: p.opacity,
          background: 'linear-gradient(135deg, rgba(96,165,250,0.8), rgba(192,132,252,0.8))',
          boxShadow: '0 0 4px rgba(99,102,241,0.4)',
          animation: getReducedMotion() ? 'none' : `orbFloat ${p.duration}s ease-in-out ${p.delay}s infinite`,
        }} />
      ))}
    </div>
  )
...
function AuroraOverlay() {
  return (
    <div className="fixed inset-0 z-[9996] pointer-events-none opacity-30" style={{
      ...
      animation: getReducedMotion() ? 'none' : 'aurora 20s ease infinite',
    }} />
  )
}
