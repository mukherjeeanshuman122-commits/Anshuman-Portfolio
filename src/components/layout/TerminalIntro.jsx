...
function TypingLine({ text, speed = 6, onComplete, prefix = '$' }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {!done && <span className="w-[2px] h-4 bg-indigo-mid animate-blink inline-block" />}
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <motion.div className="w-2 h-2 bg-indigo-mid" animate={{ scale: [1, 1.4, 1], opacity: [1, 0.5, 1] }} transition={{ duration: 0.5, repeat: Infinity }} />
          <span className="text-[10px] font-mono text-white/50 uppercase tracking-[0.15em]">
            {phase === 'checking' ? 'Clearance Protocol' : 'CLEARED'}
          </span>
        </div>
        <span className="text-sm font-mono font-bold text-indigo-mid">{percent}%</span>
      </div>

      <div className="relative h-[2px] bg-white/10 overflow-hidden">
        <motion.div className="absolute inset-y-0 left-0" style={{ background: 'linear-gradient(90deg, #4338ca, #6366f1)', width: `${percent}%` }} />
      </div>
      ...
      <div className="space-y-0.5 font-mono text-[10px] max-h-28 overflow-y-auto">
        {logs.map((log, i) => (
          <motion.div key={i} className="flex items-center gap-2" initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.08 }}>
            ...
            <span className="text-white/40 ml-auto">ok</span>
          </motion.div>
        ))}
        {phase === 'checking' && currentCheck < safetyChecks.length && (
          <div className="flex items-center gap-2">
            <motion.span className="text-indigo-mid" animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 0.4, repeat: Infinity }}>&#9679;</motion.span>
      ...
      <AnimatePresence>
        {showAccess && (
          <motion.div className="text-center pt-1" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.2 }}>
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/4" style={{ border: '1px solid rgba(99,102,241,0.15)' }}>
        ...
        <motion.div className="fixed inset-0 z-[10000] flex items-center justify-center bg-[#050505]" exit={{ opacity: 0 }} transition={{ duration: 0.3 }}>
          ...
          <motion.div className="w-full max-w-2xl mx-4" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
            <div className="overflow-hidden ide-window">
              <div className="flex items-center justify-between px-3 py-2" style={{ background: '#1a1a1a', borderBottom: '1px solid rgba(99,102,241,0.08)' }}>
                <div className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full bg-indigo-mid/70" />
                  <span className="w-2.5 h-2.5 rounded-full bg-white/15" />
                  <span className="w-2.5 h-2.5 rounded-full bg-white/15" />
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-white/50 font-mono tracking-wider">case-file@forensic ~ $</span>
                </div>
              ...
              <div ref={containerRef} className="bg-[#0a0a0a] p-5 font-mono text-[11px] leading-relaxed max-h-[350px] overflow-y-auto relative">
                {visibleLines.map((line) => {
                  if (line.type === 'system') {
                    return (
                      <div key={line.id} className="text-indigo-mid/60 my-0.5 text-[9px] tracking-[0.1em]">
                        {line.text ? `[${line.text}]` : ''}
                      </div>
                    )
                  }
                ...
                {phase === 'safety' && (
                  <motion.div className="mt-3 pt-3" style={{ borderTop: '1px solid rgba(99,102,241,0.08)' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.15 }}>
