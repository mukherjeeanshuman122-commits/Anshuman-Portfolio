      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {technologies.slice(0, 6).map((tech) => (
            <span key={tech.name} className="px-3 py-1 text-xs font-medium text-white/60 hover:text-bone hover:border-blood/30 transition-all duration-200"
      ...
      <div className="space-y-4">
        {aboutContent.experience.map((exp, i) => (
          <div key={i} className="space-y-1 pl-4 transition-colors duration-300"
            style={{ borderLeft: '1px solid rgba(139,32,32,0.2)' }}
          >
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
          {technologies.map((tech) => (
            <div key={tech.name} className="flex flex-col items-center gap-1.5 p-2.5 transition-all duration-200 group hover:scale-105 hover:border-blood/20"
              style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}
            >
              <div className="w-9 h-9 flex items-center justify-center text-xs font-bold bg-white/5 text-white/55 group-hover:text-white/80 group-hover:bg-blood/10 transition-all duration-200">
                {tech.name.charAt(0)}
              </div>
      ...
      <div className="space-y-2">
        {['Master Next.js & TypeScript', 'Build AI-powered web apps', 'Contribute to open source', 'Learn Rust & WebAssembly'].map((goal, i) => (
          <div key={i} className="flex items-center gap-2 group">
            <span className="w-1.5 h-1.5 bg-white/25 group-hover:bg-blood/60 transition-all duration-200" />
            ...
            <div key={item.id} className={sizeClasses[item.size]}
              ...
              <GlowCard tilt className="h-full p-6 group card-hover"
                ...
                <div className="flex items-center gap-2 mb-4">
                  <div className="text-white/45 group-hover:text-blood/80 transition-colors duration-200">{item.icon}</div>
