    ...
    content: (
      <div className="space-y-4">
        {aboutContent.experience.map((exp, i) => (
          <div key={i} className="space-y-1 pl-4 transition-all duration-400 !hover:border-indigo-mid/40"
            style={{ borderLeft: '1px solid rgba(99,102,241,0.25)' }}
          >
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-white/80">{exp.role}</span>
              <span className="text-xs text-white/55 font-mono">{exp.period}</span>
            </div>
            <p className="text-xs text-white/50">{exp.company}</p>
            <p className="text-xs text-white/55 mt-1">{exp.description}</p>
          </div>
        ))}
      </div>
    ),
      ...
      <div className="space-y-3">
        {aboutContent.education.map((edu, i) => (
          <div key={i} className="space-y-1">
            ...
            <p className="text-xs text-white/55 font-mono">{edu.period}</p>
            <p className="text-xs text-white/55">{edu.description}</p>
          </div>
        ))}
      </div>
    ),
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
          {technologies.map((tech, i) => {
            ...
            return (
              <div key={tech.name} className="flex flex-col items-center gap-1.5 p-2.5 transition-all duration-400 group hover:scale-105 !hover:border-indigo-mid/20"
                style={{
                ...
                <span className="text-[10px] text-white/55 text-center leading-tight">{tech.name}</span>
              </div>
            )
          })}
        </div>
      ...
      <div className="space-y-3">
        {[
          ...
          <div key={i} className="flex items-center justify-between group">
            ...
            <span className="text-sm font-bold text-white/70 group-hover:text-bone transition-colors duration-400 font-mono">{item.value}</span>
          </div>
        ))}
      </div>
    ),
      ...
      <div className="space-y-2">
        {['Master Next.js & TypeScript', 'Build AI-powered web apps', 'Contribute to open source', 'Learn Rust & WebAssembly'].map((goal, i) => (
          <div key={i} className="flex items-center gap-2 group">
            <span className="w-1.5 h-1.5 group-hover:shadow-neon-blue transition-all duration-400"
              style={{
                background: 'rgba(99,102,241,0.35)',
                boxShadow: '0 0 4px rgba(99,102,241,0.2)',
              }} />
            <span className="text-xs text-white/55 group-hover:text-white/75 transition-colors duration-400">{goal}</span>
          </div>
        ))}
      </div>
    ),
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      cardsRef.current.forEach((card, i) => {
        ...
        gsap.fromTo(card,
          { opacity: 0, y: 40 },
          { opacity: 1, y: 0, duration: 0.7, delay: i * 0.08, ease: [0.16, 1, 0.3, 1],
    ...
    <section id="about" className="relative py-32" ref={sectionRef}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-16 section-header">
          <span className="section-label" style={{ animation: 'fadeInUp 0.8s cubic-bezier(0.16,1,0.3,1) 0.1s both' }}>About</span>
          <h2 className="text-4xl sm:text-5xl font-bold mt-4" style={{ animation: 'fadeInUp 0.8s cubic-bezier(0.16,1,0.3,1) 0.2s both' }}>Developer <span className="gradient-text">Profile</span></h2>
          <p className="text-white/55 mt-4 max-w-xl mx-auto" style={{ animation: 'fadeInUp 0.8s cubic-bezier(0.16,1,0.3,1) 0.3s both' }}>Everything you need to know about my journey, skills, and what drives me.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          {bentoItems.map((item, i) => (
            <div key={item.id} className={sizeClasses[item.size]}
              ref={el => cardsRef.current[i] = el}>
              <AnimatedGradientBorder tilt className="h-full p-6 group card-hover"
                style={{
                  background: 'rgba(10,15,46,0.6)',
                  border: '1px solid rgba(99,102,241,0.08)',
                }}>
                <div className="flex items-center gap-2 mb-4">
                  <div className="text-white/55 group-hover:text-indigo-mid/80 transition-colors duration-400">{item.icon}</div>
                  <h3 className="text-sm font-semibold text-white/70 group-hover:text-white/90 transition-colors duration-400 font-mono">{item.title}</h3>
                </div>
                {item.content}
