  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg className="w-full h-full -rotate-90" viewBox={`0 0 ${size} ${size}`}>
        <defs><linearGradient id="achieveGrad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#3b82f6" /><stop offset="100%" stopColor="#8b5cf6" /></linearGradient></defs>
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="rgba(255,255,255,0.04)" strokeWidth={strokeWidth} />
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="url(#achieveGrad)" strokeWidth={strokeWidth}
...
function Sparkline() {
  ...
  return (
    <svg className="w-full h-8 mt-2" viewBox="0 0 100 50" preserveAspectRatio="none">
      <defs><linearGradient id="spark-danger" x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor="#6366f1" stopOpacity="0.2" /><stop offset="100%" stopColor="#6366f1" stopOpacity="0" /></linearGradient></defs>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {achievementList.map((item, i) => (
            <div key={item.label} ref={el => cardsRef.current[i] = el}>
              <GlowCard tilt className="rounded-2xl p-6 text-center group card-hover" style={{ background: 'rgba(10,15,46,0.6)', border: '1px solid rgba(99,102,241,0.08)' }}>
                <div className="relative inline-flex items-center justify-center mb-4">
                  <CircularProgress value={item.target} max={item.target * 1.2} size={80} strokeWidth={3} />
                  <div className="absolute inset-0 flex items-center justify-center text-indigo-mid/60">{item.svg}</div>
                </div>
