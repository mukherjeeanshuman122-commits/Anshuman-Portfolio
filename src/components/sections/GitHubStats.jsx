  ...
  return (
    <section ref={sectionRef} className="relative py-24">
      <div className="max-w-5xl mx-auto px-6 lg:px-8">
        ...
        <div className="stats-grid rounded-2xl p-8" style={{ background: 'rgba(10,15,46,0.6)', border: '1px solid rgba(99,102,241,0.08)' }}>
          {loading ? (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
              {[1,2,3,4].map(i => (
                <div key={i} className="text-center space-y-2 animate-pulse">
                  <div className="w-10 h-10 mx-auto bg-indigo-500/5 rounded-xl" />
                  <div className="h-6 w-12 mx-auto bg-indigo-500/5 rounded" />
                  <div className="h-3 w-16 mx-auto bg-indigo-500/5 rounded" />
                </div>
              ))}
            </div>
          ) : stats ? (
            <>
              <div className="flex items-center gap-4 mb-6 pb-6" style={{ borderBottom: '1px solid rgba(99,102,241,0.08)' }}>
                <img src={stats.avatar_url} alt={stats.login} className="w-14 h-14 rounded-xl" style={{ border: '2px solid rgba(99,102,241,0.25)' }} />
                <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                {statItems.map((item) => (
                  <div key={item.label} className="text-center group">
                    <div className="text-indigo-mid/60 group-hover:text-bone/60 mb-1 transition-colors">{item.svg}</div>
