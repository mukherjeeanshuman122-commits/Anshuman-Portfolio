...
function PricingCard({ plan, index }) {
  ...
  return (
    <div
      ...
      className={`pricing-card relative flex flex-col rounded-2xl transition-all duration-400 ${plan.popular ? 'pricing-popular scale-[1.02]' : ''}`}
      style={{
        background: plan.popular
          ? 'linear-gradient(180deg, rgba(99,102,241,0.1) 0%, rgba(99,102,241,0.03) 100%)'
          : hovered
          ? 'rgba(99,102,241,0.03)'
          : 'rgba(99,102,241,0.015)',
        border: `1px solid ${plan.popular ? 'rgba(99,102,241,0.2)' : hovered ? 'rgba(99,102,241,0.08)' : 'rgba(99,102,241,0.04)'}`,
      }}
    >
      {plan.popular && (
        <div className="absolute -top-px left-0 right-0 h-[2px] rounded-t-2xl overflow-hidden">
          <div className="w-full h-full" style={{ background: 'linear-gradient(90deg, transparent, #6366f1, #818cf8, #6366f1, transparent)' }} />
        </div>
      )}
      ...
      {plan.popular && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-10">
          <div className="px-4 py-1.5 text-[10px] font-mono font-bold uppercase tracking-wider rounded-full flex items-center gap-1.5"
            style={{ background: 'linear-gradient(135deg, #6366f1, #818cf8)', color: '#E8E4D9', boxShadow: '0 0 20px rgba(99,102,241,0.4), 0 0 40px rgba(99,102,241,0.15)' }}>
      ...
      <div className="p-6 pb-4 flex-1">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-400"
            style={{
              background: plan.popular ? 'rgba(99,102,241,0.15)' : 'rgba(99,102,241,0.04)',
              border: `1px solid ${plan.popular ? 'rgba(99,102,241,0.25)' : 'rgba(99,102,241,0.06)'}`,
              color: plan.popular ? '#818cf8' : 'rgba(255,255,255,0.35)',
            }}>
            {plan.icon}
          </div>
          <div>
            <h4 className="text-sm font-mono font-semibold text-white/80">{plan.name}</h4>
            <p className="text-[10px] font-mono text-white/50">{plan.period}</p>
          </div>
        </div>

        <div className="mb-2">
          <div className="flex items-baseline gap-1">
            <span className="text-sm font-mono text-white/50 self-start mt-1">₹</span>
            <span className="text-4xl sm:text-5xl font-bold tracking-tight" style={{ color: plan.popular ? '#E8E4D9' : 'rgba(255,255,255,0.85)' }}>
              {plan.price}
            </span>
          </div>
          <p className="text-[11px] font-mono text-white/50 mt-1.5 leading-relaxed">{plan.tagline}</p>
        </div>

        <div className="my-4 h-[1px]" style={{ background: plan.popular ? 'linear-gradient(90deg, transparent, rgba(99,102,241,0.2), transparent)' : 'linear-gradient(90deg, transparent, rgba(99,102,241,0.06), transparent)' }} />
            ...
            <div key={i} className="flex items-start gap-2.5 group/feat">
              {feature.highlight ? (
                <div className="w-4 h-4 rounded-full flex items-center justify-center shrink-0 mt-0.5"
                  style={{ background: plan.popular ? 'rgba(99,102,241,0.2)' : 'rgba(99,102,241,0.06)' }}>
                  <svg className="w-2.5 h-2.5" viewBox="0 0 24 24" fill="none" stroke={plan.popular ? '#818cf8' : 'rgba(255,255,255,0.3)'} strokeWidth="3">
                    <polyline points="20 6 9 17 4 12" />
                  </svg>
                </div>
              ) : (
                <svg className="w-3.5 h-3.5 mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke={plan.popular ? 'rgba(99,102,241,0.5)' : 'rgba(255,255,255,0.35)'} strokeWidth="2">
                  <polyline points="20 6 9 17 4 12" />
                </svg>
              )}
              <span className={`text-[11px] font-mono leading-relaxed ${feature.highlight ? (plan.popular ? 'text-white/70 font-medium' : 'text-white/55') : 'text-white/55'}`}>
                {feature.text}
              </span>
            </div>
          ))}
      ...
      <div className="p-6 pt-4">
        <button
          ...
          onClick={handleBuy}
          className="pricing-btn relative w-full py-3.5 rounded-xl text-xs font-mono font-semibold tracking-wider uppercase overflow-hidden transition-all duration-400 group/btn"
        >
          <div className="absolute inset-0 rounded-xl transition-all duration-400"
            style={{
              background: plan.popular
                ...
                : hovered ? 'rgba(99,102,241,0.06)' : 'rgba(99,102,241,0.03)',
              border: `1px solid ${plan.popular ? 'rgba(99,102,241,0.35)' : 'rgba(99,102,241,0.08)'}`,
            }} />
          ...
          <span className="relative z-10 flex items-center justify-center gap-2" style={{ color: plan.popular ? '#E8E4D9' : 'rgba(255,255,255,0.65)' }}>
            <span>Get Started</span>
            <svg className="w-3.5 h-3.5 group-hover/btn:translate-x-1 transition-transform duration-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
export default function Pricing() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(sectionRef.current.querySelector('.pricing-tabs'),
        { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.5, delay: 0.2, ease: [0.16, 1, 0.3, 1],
          scrollTrigger: { trigger: sectionRef.current, start: 'top 80%', once: true } }
      )
    }, sectionRef)
    return () => ctx.revert()
  ...
  useEffect(() => {
    ...
    gsap.fromTo(cards,
      ...
      { opacity: 1, y: 0, scale: 1, duration: 0.6, stagger: 0.12, ease: [0.16, 1, 0.3, 1] }
    )
  }, [activeTab])
    ...
    <section id="pricing" className="relative py-32" ref={sectionRef}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-14 pricing-header">
          ...
          <p className="text-white/55 mt-4 max-w-lg mx-auto text-sm">
          ...
          <div className="inline-flex liquid-glass rounded-2xl p-1.5 gap-1">
            {categories.map((cat) => (
              <button
                ...
                className="relative flex items-center gap-2 px-5 sm:px-6 py-3 rounded-xl text-xs font-mono font-medium transition-all duration-400"
                style={{
                  color: activeTab === cat.id ? '#E8E4D9' : 'rgba(255,255,255,0.55)',
                }}
              >
                {activeTab === cat.id && (
                  <div className="absolute inset-0 rounded-xl" style={{ background: 'rgba(99,102,241,0.15)', border: '1px solid rgba(99,102,241,0.2)' }} />
                )}
                <span className="relative z-10 flex items-center gap-2">
                  <span className={activeTab === cat.id ? 'text-blood' : 'text-white/45'}>{cat.icon}</span>
        ...
        <div className="mt-16 grid sm:grid-cols-3 gap-4 max-w-4xl mx-auto">
          {[
            ...
            <div key={i} className="liquid-glass rounded-xl p-5 text-center group hover:border-blood/15 transition-all duration-400">
              ...
              <p className="text-[10px] font-mono text-white/50">{item.desc}</p>
            </div>
          ))}
        </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-1.5">
                {[
                  ...
                  <p key={i} className="text-[10px] font-mono text-white/50">{item}</p>
                ))}
              </div>
              <p className="text-[10px] font-mono text-white/45 mt-3 leading-relaxed">
