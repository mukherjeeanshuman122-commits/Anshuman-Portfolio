...
function TerminalDate({ date }) { const d = new Date(date); return <span className="font-mono text-[10px] text-white/40">{d.toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: '2-digit' })}</span> }
    ...
    <section id="blog" className="relative py-32" ref={sectionRef}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-16 section-header">
          ...
          <p className="text-white/50 mt-4 max-w-xl mx-auto">Thoughts on development, design, and technology.</p>
        </div>
          ...
          {posts.map((post, i) => (
            <article key={post.title} className="group overflow-hidden card-hover"
              ...
              <div className="px-4 py-2.5 flex items-center gap-2"
                style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(10,10,10,0.95)' }}>
                <span className="w-2.5 h-2.5 bg-blood/60" /><span className="w-2.5 h-2.5 bg-white/15" /><span className="w-2.5 h-2.5 bg-white/15" />
                <span className="ml-2 text-[10px] font-mono text-white/35">post.md</span>
              </div>
              <div className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <span className="px-2 py-0.5 text-[10px] font-mono text-white/55 bg-white/5">#{post.tag}</span>
                ...
                <div className="flex items-center justify-between pt-2" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                  <span className="text-[10px] text-white/35 font-mono">{post.readTime} read</span>
