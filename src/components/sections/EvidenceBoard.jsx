      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-8">
        ...
        <div className="eb-board relative grid grid-cols-2 md:grid-cols-4 gap-6 p-8 rounded-2xl"
          ...
          {pinnedProjects.map((project) => (
            ...
            >
              <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-indigo-mid/40 border border-indigo-mid/20" />

              <div className="text-[10px] font-mono text-white/25 mb-2 flex items-center gap-1">
                <span className="w-1 h-1 bg-indigo-mid/40" />
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                {project.tags.slice(0, 2).map(tag => (
                  <span key={tag} className="text-[8px] px-1.5 py-0.5 bg-indigo-500/10 text-white/35 font-mono">{tag}</span>
                ))}
              </div>
          ...
          <div className="absolute inset-0 pointer-events-none" style={{
            background: 'repeating-linear-gradient(-45deg, transparent, transparent 40px, rgba(99,102,241,0.02) 40px, rgba(99,102,241,0.02) 41px)',
          }} />
        </div>
      </div>
    </section>
