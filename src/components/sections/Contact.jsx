  ...
  return (
    <div className="relative group">
      <div className="absolute -inset-px rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
        style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.12), transparent 40%, rgba(139,92,246,0.08))' }} />
      <div className="relative">
        <div className="absolute left-4 top-1/2 -translate-y-1/2 transition-all duration-400 pointer-events-none"
          style={{ color: focused ? 'rgba(129,140,248,0.9)' : 'rgba(226,232,240,0.4)', transform: isActive ? 'translateY(-24px) scale(0.7)' : 'translateY(-50%) scale(1)', left: '16px', transformOrigin: 'left center' }}>
          {icon}
        </div>
        <input
          ...
          className="contact-input w-full pl-11 pr-4 pt-5 pb-2 text-sm font-mono rounded-xl transition-all duration-400"
        />
        <label className="absolute left-11 transition-all duration-400 pointer-events-none font-mono"
          style={{
            top: isActive ? '10px' : '50%',
            transform: isActive ? 'translateY(0) scale(0.75)' : 'translateY(-50%) scale(1)',
            color: focused ? 'rgba(129,140,248,0.95)' : 'rgba(226,232,240,0.5)',
            fontSize: '10px',
            letterSpacing: '0.1em',
        ...
        <div className="absolute bottom-0 left-4 right-4 h-[1px] overflow-hidden rounded-full">
          <div className="h-full transition-all duration-500 ease-out"
            style={{
              width: focused ? '100%' : '0%',
              background: 'linear-gradient(90deg, rgba(59,130,246,0.6), rgba(99,102,241,0.4), rgba(139,92,246,0.5))',
              boxShadow: focused ? '0 0 8px rgba(99,102,241,0.3)' : 'none',
            }} />
        </div>
      </div>
    </div>
  ...
  return (
    <div className="relative group">
      <div className="absolute -inset-px rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
        style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.12), transparent 40%, rgba(139,92,246,0.08))' }} />
      <div className="relative">
        <div className="absolute left-4 top-5 transition-all duration-400 pointer-events-none"
          style={{ color: focused ? 'rgba(129,140,248,0.9)' : 'rgba(226,232,240,0.4)' }}>
        ...
        <textarea
          ...
          rows={4}
          maxLength={maxLength}
          className="contact-input w-full pl-11 pr-4 pt-5 pb-2 text-sm font-mono rounded-xl transition-all duration-400 resize-none"
        />
        <label className="absolute left-11 transition-all duration-400 pointer-events-none font-mono"
          style={{
      ...
      <div className="flex justify-between items-center mt-2 px-1">
        ...
        <div className="flex items-center gap-2">
          <div className="w-16 h-[2px] rounded-full overflow-hidden"
            style={{ background: 'rgba(99,102,241,0.1)' }}>
            <div className="h-full rounded-full transition-all duration-400"
              style={{
                width: `${Math.min(charPercent, 100)}%`,
                background: charPercent > 90 ? 'rgba(239,68,68,0.7)' : charPercent > 70 ? 'rgba(234,179,8,0.6)' : 'linear-gradient(90deg, #3b82f6, #8b5cf6)',
              }} />
          </div>
          <span className="text-[9px] font-mono" style={{ color: charPercent > 90 ? 'rgba(239,68,68,0.7)' : 'rgba(226,232,240,0.4)' }}>
            {charCount}/{maxLength}
          </span>
        </div>
      </div>
...
function SubmitButton({ sending, submitted }) {
  ...
  return (
    <button ref={btnRef} type="submit" disabled={sending || submitted}
      className="submit-btn relative w-full py-4 rounded-xl font-mono text-sm font-semibold tracking-wide overflow-hidden transition-all duration-400 disabled:cursor-not-allowed group">
      <div className="absolute inset-0 rounded-xl" style={{
        background: submitted
          ? 'linear-gradient(135deg, rgba(34,197,94,0.15), rgba(34,197,94,0.05))'
          : 'linear-gradient(135deg, rgba(99,102,241,0.2), rgba(139,92,246,0.12), rgba(59,130,246,0.15))',
        border: `1px solid ${submitted ? 'rgba(34,197,94,0.25)' : 'rgba(99,102,241,0.35)'}`,
      }} />
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-xl"
        style={{ background: 'radial-gradient(circle at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(99,102,241,0.15), transparent 60%)' }} />
      <div className="absolute inset-0 rounded-xl overflow-hidden">
        <div className="absolute -top-1/2 -left-1/2 w-[200%] h-[200%] opacity-0 group-hover:opacity-100 transition-opacity duration-700"
          style={{ background: 'conic-gradient(from 0deg, transparent, rgba(99,102,241,0.12), rgba(139,92,246,0.08), transparent 30%)', animation: 'spin 4s linear infinite' }} />
      </div>
          ...
          <>
            <div className="relative w-5 h-5">
              ...
              <div className="absolute inset-0 border-2 border-transparent rounded-full"
                style={{ borderTopColor: 'rgba(99,102,241,0.7)', animation: 'spin 0.8s linear infinite' }} />
            </div>
            <span className="text-white/70">Sending Evidence...</span>
          </>
        ) : (
          <>
            <svg className="w-4 h-4 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <line x1="22" y1="2" x2="11" y2="13" />
              <polygon points="22 2 15 22 11 13 2 9 22 2" />
            </svg>
            <span>Transmit Message</span>
            <svg className="w-4 h-4 group-hover:translate-x-1 transition-transform duration-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
...
function ContactCard({ icon, label, value, href, onClick, delay = 0 }) {
  ...
  return (
    <div ref={cardRef}
      ...
      className="contact-card group relative p-4 rounded-xl cursor-pointer transition-all duration-400"
      style={{
        background: hovered ? 'rgba(99,102,241,0.06)' : 'rgba(99,102,241,0.02)',
        border: `1px solid ${hovered ? 'rgba(99,102,241,0.2)' : 'rgba(99,102,241,0.05)'}`,
      }}>
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-400"
          style={{
            background: hovered ? 'rgba(99,102,241,0.15)' : 'rgba(99,102,241,0.04)',
            border: `1px solid ${hovered ? 'rgba(99,102,241,0.25)' : 'rgba(99,102,241,0.08)'}`,
          }}>
          {icon}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-[9px] font-mono uppercase tracking-wider text-white/45 mb-0.5">{label}</p>
          <p className="text-xs font-mono text-white/65 truncate group-hover:text-white/85 transition-colors">{value}</p>
        </div>
        <div className="opacity-0 group-hover:opacity-100 transition-all duration-400 translate-x-1 group-hover:translate-x-0">
          {onClick ? (
            <svg className="w-3.5 h-3.5 text-indigo-mid/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  return (
    <div ref={overlayRef} className="absolute inset-0 z-50 flex items-center justify-center rounded-2xl overflow-hidden"
      ...
      <div className="text-center relative z-10">
        ...
        <p className="success-text text-[10px] font-mono text-white/50 mt-1">Response incoming within 24hrs</p>
      </div>
    </div>
  )
}
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo(sectionRef.current.querySelector('.contact-header'),
        { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.8, ease: [0.16, 1, 0.3, 1],
          scrollTrigger: { trigger: sectionRef.current, start: 'top 80%', once: true } }
      )
      gsap.fromTo(sectionRef.current.querySelector('.contact-info-panel'),
        { opacity: 0, x: -40 }, { opacity: 1, x: 0, duration: 0.7, ease: [0.16, 1, 0.3, 1],
      ...
      gsap.fromTo(sectionRef.current.querySelector('.contact-form-panel'),
        { opacity: 0, x: 40 }, { opacity: 1, x: 0, duration: 0.7, delay: 0.15, ease: [0.16, 1, 0.3, 1],
      ...
      cards.forEach((card, i) => {
        gsap.fromTo(card, { opacity: 0, y: 15 }, { opacity: 1, y: 0, duration: 0.4, delay: 0.2 + i * 0.08, ease: [0.16, 1, 0.3, 1],
          ...
          <div className="lg:col-span-4 space-y-6 contact-info-panel">
            <div className="liquid-glass rounded-2xl p-6 space-y-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-white/50 font-mono">{personalInfo.responseTime}</p>
                </div>
              </div>
            ...
            <div className="liquid-glass rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <svg className="w-3.5 h-3.5 text-indigo-mid/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" />
                </svg>
                <span className="text-[9px] font-mono uppercase tracking-wider text-white/50">Quick Info</span>
              </div>
              <div className="space-y-2">
                {[
                  ...
                  <div key={item.label} className="flex justify-between items-center py-1.5"
                    ...
                    <span className="text-[9px] font-mono text-white/45 uppercase tracking-wider">{item.label}</span>
                    <span className="text-[10px] font-mono text-white/65">{item.value}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="liquid-glass rounded-2xl p-8 relative overflow-hidden">
              ...
              <div className="flex items-center gap-2 mb-6">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center"
                  ...
                  <svg className="w-4 h-4 text-indigo-mid/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                ...
                <div>
                  ...
                  <p className="text-[9px] font-mono text-white/45">Fill in the evidence log</p>
                </div>
              </div>
              ...
              <div className="mt-6 pt-5 flex items-center justify-center gap-2"
                ...
                <span className="text-[9px] font-mono text-white/45">End-to-end encrypted • Your data stays private</span>
              </div>
            </div>
          </div>
        </div>
