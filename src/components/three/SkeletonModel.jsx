  ...
  return (
    <group position={midpoint} quaternion={rotation}>
      <mesh>
        ...
        <meshStandardMaterial color="#C2C2B4" roughness={0.65} metalness={0.12} />
      </mesh>
    </group>
  )
}
...
function Joint({ position, size = 0.04 }) {
  return (
    <mesh position={position}>
      ...
      <meshStandardMaterial color="#B0B0A0" roughness={0.55} metalness={0.15} />
    </mesh>
  )
}
  ...
  return (
    <group position={[0, 1.72, 0]}>
      <mesh>
        ...
        <meshStandardMaterial color="#D0D0C0" roughness={0.45} metalness={0.1} />
      </mesh>
      <mesh position={[0, -0.06, 0.06]}>
        <boxGeometry args={[0.1, 0.08, 0.1]} />
        <meshStandardMaterial color="#C8C8B8" roughness={0.5} metalness={0.1} />
      </mesh>
      <mesh position={[-0.04, 0.02, 0.1]}>
        <sphereGeometry args={[0.025, 8, 8]} />
        <meshStandardMaterial color="#0A0A0A" roughness={0.9} emissive="#3a1fb8" emissiveIntensity={eyeGlow * 0.4} />
      </mesh>
      <mesh position={[0.04, 0.02, 0.1]}>
        <sphereGeometry args={[0.025, 8, 8]} />
        <meshStandardMaterial color="#0A0A0A" roughness={0.9} emissive="#3a1fb8" emissiveIntensity={eyeGlow * 0.4} />
      ...
      <mesh position={[0, -0.09, 0.02]} rotation={[0.3, 0, 0]}>
        ...
        <meshStandardMaterial color="#C2C2B4" roughness={0.6} metalness={0.1} />
      </mesh>
    </group>
  )
}
      ...
      {ribs.map((rib, i) => (
        <group key={i}>
          <mesh position={[-rib.width / 2, rib.y, rib.curve / 2]} rotation={[0, 0, 0.3]}>
            ...
            <meshStandardMaterial color="#B8B8A8" roughness={0.6} metalness={0.1} />
          </mesh>
