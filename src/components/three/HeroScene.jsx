...
function CameraDrift({ isLowEnd }) {
  const { camera } = useThree()
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime()
    const speed = isLowEnd ? 0.04 : 0.08
    const amplitude = isLowEnd ? 0.08 : 0.15
    camera.position.x = Math.sin(t * speed) * amplitude
    camera.position.y = (isLowEnd ? 0.2 : 0.3) + Math.cos(t * speed * 0.75) * (amplitude * 0.67)
    camera.lookAt(0, 0, 0)
  })
  return null
}

function Fog({ isLowEnd }) {
...
function DebrisParticles({ count }) {
...
function WireframeRings({ isLowEnd }) {
  const groupRef = useRef()
  useFrame(({ clock }) => {
    if (!groupRef.current) return
    const speed = isLowEnd ? 0.015 : 0.03
    groupRef.current.rotation.x = clock.getElapsedTime() * speed
    groupRef.current.rotation.z = clock.getElapsedTime() * (speed * 0.5)
  })
  return (
    <group ref={groupRef}>
      <mesh rotation={[Math.PI / 2, 0, 0]}>
        <torusGeometry args={[2.8, 0.003, isLowEnd ? 4 : 8, isLowEnd ? 30 : 60]} />
...
export default function HeroScene() {
  const { threeDpr, threeParticleCount, isLowEnd } = useDevicePerformance()
  return (
    <Canvas camera={{ position: [0, isLowEnd ? 0.2 : 0.3, 4.5], fov: 40 }} dpr={threeDpr}>
      <CameraDrift isLowEnd={isLowEnd} />
      <Fog isLowEnd={isLowEnd} />
      ...
      <SkeletonModel />
      <WireframeRings isLowEnd={isLowEnd} />
      <DebrisParticles count={threeParticleCount} />
    </Canvas>
  )
}
